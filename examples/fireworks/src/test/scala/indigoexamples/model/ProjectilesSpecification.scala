...
class ProjectilesSpecification extends Properties("Projectiles") {
  ...
  property("toScreenSpace should always put vertices on the screen") = Prop.forAll(vertexClamped(-1, 1, 0, 1)) {
    vertex: Vertex =>
      val position: Point =
  ...
  property("arc signal should always produce a value inside the beziers bounds") = Prop.forAll {
    (target: Vertex, time: Seconds) =>
  ...
  property("Over time, the signal should always generate values moving closer to the target") =
