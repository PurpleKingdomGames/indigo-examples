...
class RocketSpecification extends Properties("Rocket") {
  ...
  property("control points are always in order [start, mid, target]") = Prop.forAll(diceGen, launchPadVertexGen) {
    (dice: Dice, launch: Vertex) =>
  ...
  property("arc mid control point y is always in line with the target y position") =
    Prop.forAll(diceGen, launchPadVertexGen) { (dice: Dice, launch: Vertex) =>
      Prop.forAll(rocketTargetVertexGen(launch)) { target =>
        Rocket.createArcControlVertices(dice, launch)(target) match {
          case NonEmptyList(_, m :: e :: Nil) =>
            m.y === e.y
  ...
  property("arc mid control point x is always more than half way towards the target") =
    Prop.forAll(diceGen, launchPadVertexGen) { (dice: Dice, launch: Vertex) =>
      Prop.forAll(rocketTargetVertexGen(launch)) { target =>
        val vertices =
  ...
  property("able to generate a good target vertex based on a start point") = Prop.forAll(diceGen, launchPadVertexGen) {
    (dice: Dice, launch: Vertex) =>
      val target: Vertex =
  ...
  property("creates between 5 and 8 flares that all share the rockets end point and blast radius") = Prop.forAll {
    (dice: Dice, start: Vertex) =>
